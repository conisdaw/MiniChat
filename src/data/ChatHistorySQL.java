...
public class ChatHistorySQL {
    // 写入聊天记录 群聊需传入groupID
    public static void insertChatMessage(Connection conn, boolean isGroup, String groupID,
                                         String peerID, String messageType, String content,
                                         boolean isSent, String ip, int port) throws SQLException {
        String sql;
        if (isGroup) {
            sql = "INSERT INTO GroupMessages (group_id, sender_id, message_type, content, is_sent, ip_address, port) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?)";
        } else {
            sql = "INSERT INTO SingleChatHistory (peer_id, message_type, message, is_sent, ip_address, port) " +
                    "VALUES (?, ?, ?, ?, ?, ?)";
        }
    ...
    // 读取聊天记录 群聊需传入groupID
    public static ResultSet readChatHistory(Connection conn, boolean isGroup, String targetID) throws SQLException {
        String sql;
        if (isGroup) {
